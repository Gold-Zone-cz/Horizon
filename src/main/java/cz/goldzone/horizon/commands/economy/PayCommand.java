...
public class PayCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!PayToggleCommand.isPayEnabled(target)) {
            player.sendMessage(Lang.getPrefix("Economy") + "<red>The player has disabled payments. You cannot send money to them.");
            return false;
        }
        ...
        MoneyManager senderMoney = new MoneyManager(player);
        MoneyManager targetMoney = new MoneyManager(target);

        if (senderMoney.getAmount() < amount) {
            player.sendMessage(Lang.getPrefix("Economy") + "<red>You don't have enough money!");
            return false;
        }

        senderMoney.subtract((int) amount);
        targetMoney.add((int) amount);

        player.sendMessage(Lang.getPrefix("Economy") + "<gray>You have sent <red>$" + amount + " <gray> to <red>" + target.getName());
        target.sendMessage(Lang.getPrefix("Economy") + "<red>" + player.getName() + "<gray> has sent you <red>$" + amount);
